	...
	*(WORD*)(pdest + 1) = 0; // use default window class
	...
	for (i = 0; i < m_dlgTempl.cdit; i++)
	{
		...
		*(WORD*)pdest = (WORD)m_pDlgItemArray[i]->m_controltype;	// atom value for the control
		...
		memcpy(pchCaption, m_pDlgItemArray[i]->m_szCaption, nChars * sizeof(TCHAR));
		int nActualChars = nChars;
#else
		int nActualChars = MultiByteToWideChar(CP_ACP, 0,
			(LPCSTR)m_pDlgItemArray[i]->m_szCaption, -1, pchCaption, nChars);
#endif
	...
	_ASSERTE(pdest - pBuffer == nBufferSize); // just make sure we did not overrun the heap
